...
* changed README and added JavaDoc
* changed cli implementation
	** created AbstractClioption object
	** implemented all standard options in stand-alone classes, with additional optional/required feature for all but help (automatically optional)
	** changed StandardOptions from enum to abstract class and added the new option implementation as statics
	** moved all cli related classes in package cli
	** added +getAdditionalDescription()+ to ExecS_CliOption for more specific help options
* renamed ExecS_Cli to ExecS_CliParser
* changes in ExecS and ExecutableService
	** added "--help" and "-?" to automatic help (-h) handling in ExecS
	** renamed +getName()+ to +getServiceName()+
	** added automatic CLI parsing as default implementation to +ExecutableService.executeService()+
	** added +serviceHelpScreen(String)+ to ExecutableService for more specific help options
